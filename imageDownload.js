const playImage = async () => {
  ...
  for (let i = 0; i <= imageUrl.length - 1; i++) {
    const image = await fetch(imageUrl[i]);
    const imageBlog = await image.blob();
    const imageURL = URL.createObjectURL(imageBlog);

    const link = document.createElement('a');
    link.href = imageURL;
    const arr = imageUrl[i].split('/');
    link.download = `${arr[arr.length - 3] + arr[arr.length - 2] + arr[arr.length - 1]
      }`;
